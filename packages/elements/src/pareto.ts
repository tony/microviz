...
const SPEC_TYPE = "pareto";

export class MicrovizPareto extends HTMLElement {
  ...
  #computeFromAttributes(): RenderModel {
    ...
    const gap = this.hasAttribute("gap")
      ...
      : undefined;
    const bgOpacity = this.hasAttribute("bg-opacity")
      ? parseNumber(this.getAttribute("bg-opacity"), 0.25)
      : undefined;

    return computeModel({
      data: segments,
      size: { height, width },
      spec: { bgOpacity, gap, pad, type: SPEC_TYPE },
    });
  }
}
