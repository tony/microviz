...
const SPEC_TYPE = "cascade-steps";

export class MicrovizCascadeSteps extends HTMLElement {
  ...
  #computeFromAttributes(): RenderModel {
    ...
    const stepDecrement = this.hasAttribute("step-decrement")
      ? parseNumber(this.getAttribute("step-decrement"), 15)
      : undefined;
    const minHeightPct = this.hasAttribute("min-height-pct")
      ? parseNumber(this.getAttribute("min-height-pct"), 10)
      : undefined;

    return computeModel({
      data: segments,
      size: { height, width },
      spec: { gap, minHeightPct, pad, stepDecrement, type: SPEC_TYPE },
    });
  }
}
