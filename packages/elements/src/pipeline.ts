...
const SPEC_TYPE = "pipeline";

export class MicrovizPipeline extends HTMLElement {
  ...
  #computeFromAttributes(): RenderModel {
    ...
    const overlap = this.hasAttribute("overlap")
      ? parseNumber(this.getAttribute("overlap"), 8)
      : undefined;
    const strokeWidth = this.hasAttribute("stroke-width")
      ? parseNumber(this.getAttribute("stroke-width"), 1)
      : undefined;
    const strokeOpacity = this.hasAttribute("stroke-opacity")
      ? parseNumber(this.getAttribute("stroke-opacity"), 0.18)
      : undefined;

    return computeModel({
      data: segments,
      size: { height, width },
      spec: { overlap, pad, strokeOpacity, strokeWidth, type: SPEC_TYPE },
    });
  }
}
