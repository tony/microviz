...
const SPEC_TYPE = "stripe-density";

export class MicrovizStripeDensity extends HTMLElement {
  ...
  #computeFromAttributes(): RenderModel {
    ...
    const pad = this.hasAttribute("pad")
      ...
      : undefined;
    const stripeWidth = this.hasAttribute("stripe-width")
      ? parseNumber(this.getAttribute("stripe-width"), 2)
      : undefined;
    const minTileWidth = this.hasAttribute("min-tile-width")
      ? parseNumber(this.getAttribute("min-tile-width"), 3)
      : undefined;
    const maxTileWidth = this.hasAttribute("max-tile-width")
      ? parseNumber(this.getAttribute("max-tile-width"), 14)
      : undefined;
    const densityScale = this.hasAttribute("density-scale")
      ? parseNumber(this.getAttribute("density-scale"), 0.25)
      : undefined;
