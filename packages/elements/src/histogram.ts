...
export class MicrovizHistogram extends HTMLElement {
  ...
  #computeFromAttributes(): RenderModel {
    ...
    const bins = this.hasAttribute("bins")
      ? parseNumber(this.getAttribute("bins"), 18)
      : undefined;
    const barRadius = this.hasAttribute("bar-radius")
      ? parseNumber(this.getAttribute("bar-radius"), 0)
      : undefined;

    return computeModel({
      data: { opacities, series },
      size: { height, width },
      spec: { barRadius, bins, pad, type: SPEC_TYPE },
    });
  }
}
