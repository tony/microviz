...
const SPEC_TYPE = "split-ribbon";

export class MicrovizSplitRibbon extends HTMLElement {
  ...
  #computeFromAttributes(): RenderModel {
    ...
    const gap = this.hasAttribute("gap")
      ...
      : undefined;
    const ribbonGap = this.hasAttribute("ribbon-gap")
      ? parseNumber(this.getAttribute("ribbon-gap"), 4)
      : undefined;
    const splitAt = this.hasAttribute("split-at")
      ? parseNumber(this.getAttribute("split-at"), 0)
      : undefined;

    return computeModel({
      data: segments,
      size: { height, width },
      spec: { gap, pad, ribbonGap, splitAt, type: SPEC_TYPE },
    });
  }
}
