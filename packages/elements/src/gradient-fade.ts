...
const SPEC_TYPE = "gradient-fade";

export class MicrovizGradientFade extends HTMLElement {
  ...
  #computeFromAttributes(): RenderModel {
    ...
    const pad = this.hasAttribute("pad")
      ...
      : undefined;
    const midOpacity = this.hasAttribute("mid-opacity")
      ? parseNumber(this.getAttribute("mid-opacity"), 0.7)
      : undefined;
    const endOpacity = this.hasAttribute("end-opacity")
      ? parseNumber(this.getAttribute("end-opacity"), 0.35)
      : undefined;

    return computeModel({
      data: segments,
      size: { height, width },
      spec: { endOpacity, midOpacity, pad, type: SPEC_TYPE },
    });
  }
}
