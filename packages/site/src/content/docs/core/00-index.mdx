...
## Model overlays (post-processing hooks)

If you want to add debug marks/defs or modify a model without forking chart code:
...
const withOverlay = patchRenderModel(
  model,
  {
    defs: [
      {
        id: defId("mv-overlay-filter"),
        primitives: [{ type: "gaussianBlur", stdDeviation: 1 }],
        type: "filter",
      },
    ],
  },
