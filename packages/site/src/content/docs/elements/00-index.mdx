...
### Renderer selection (experimental HTML)

By default, elements render SVG. You can opt into the experimental HTML renderer:
...
- Other defs/effects are ignored; use SVG for full fidelity.

The same `renderer` attribute works for `<microviz-model>`:

```html
<microviz-model renderer="html"></microviz-model>
```
