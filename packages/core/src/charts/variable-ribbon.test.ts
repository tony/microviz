...
describe("variable-ribbon", () => {
  test("produces a deterministic RenderModel", () => {
    const input = {
      ...
      spec: { pad: 0, type: "variable-ribbon" as const },
    };
  ...
  test("height decreases by index and bars are centered", () => {
    const input = {
      data: [
    ...
    if (
      ...
      // Bars should be vertically centered (y + h/2 = usableH/2)
      expect(first.y + first.h / 2).toBeCloseTo(50, 1);
      expect(second.y + second.h / 2).toBeCloseTo(50, 1);
      expect(third.y + third.h / 2).toBeCloseTo(50, 1);
    }
  });
  ...
  test("respects minHeightPct option", () => {
    ...
    if (lastMark?.type === "rect") {
      expect(lastMark.h).toBeGreaterThanOrEqual(40);
    }
  });
});
