...
- ✅ **Elements wiring:** map `a11y` to `ElementInternals` (`role`, `aria-label`, `aria-description`), add SR-only summary/table, and implement roving tabindex + arrow-key focus, including focused item → `InteractionState.focusedMarkId` sync.
